...
_URL = 'https://archive.ics.uci.edu/ml/machine-learning-databases/00350/default%20of%20credit%20card%20clients.xls'


class CreditCardDefaultLoader(data_loader.DataLoader):
    """Loads the UCI Credit Card Default dataset from the internet or local storage.
    ...
    The dataset has columns:
        ...
        BILL_AMT[1-6]
        PAY_AMT[1-6]"""
    def __init__(self, only_continuous: bool = True):
        ...
        Args:
            only_continuous: If True, drops the categorical SEX column and
                             turns the categorical PAY_* columns into continuous
                             columns."""
        super().__init__(
    ...
    def process_data(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        data = data.set_index('ID').drop(
            columns=['MARRIAGE','EDUCATION']).rename(
                columns={_SOURCE_TARGET_COLUMN: 'Y', 'PAY_0': 'PAY_1'})
