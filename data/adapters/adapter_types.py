...
class RecourseAdapter(abc.ABC):
    ...
    space and continuous embedded space where recourse directions are generated.
    It also iterates recourse by converting directions in embedded space to
    human-readable instructions and interpreting recourse instructions as a
    hypothetical user would."""
    @abc.abstractmethod
    ...
    def transform_series(self, data: pd.Series) -> EmbeddedSeries:
        """Transforms data from human-readable format to an embedded continuous
        space.
        ...
        Returns:
            Transformed data."""
        return self.transform(data.to_frame().T).iloc[0]

    def inverse_transform_series(self, data: EmbeddedSeries) -> pd.Series:
        """Transforms data from an embedded continuous space to its original
        human-readable format.
        ...
        Returns:
            Inverse transformed data."""
        return self.inverse_transform(data.to_frame().T).iloc[0]
