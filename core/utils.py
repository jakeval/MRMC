...
def centroid_normalization(dir, poi, X, alpha=0.7):
    ...
    if (
        dir @ dir == 0
    ):  # if the direction is zero or very near it, return the original direction
        return dir
...
def random_perturb_dir(
    ...
    for feature in cat_features:
        ...
        if np.random.random() <= categorical_prob:
            ...
            if (new_dir[ohe_columns].to_numpy() == 0).all():
                ...
                dir_cat = ohe_columns[
                    np.argmax(new_dir[ohe_columns].to_numpy())
                ]
    ...
    if num_only:
        ...
        new_numeric_values = (
            new_numeric_values * np.linalg.norm(old_numeric_values)
        ) / np.linalg.norm(new_numeric_values)
        new_dir.loc[:, num_features] = new_numeric_values
    else:
        new_dir = (
            new_dir
            * np.linalg.norm(dir.to_numpy())
            / np.linalg.norm(new_dir.to_numpy())
        )
...
def private_alpha(dist, cutoff=0.5, degree=2):
    return (
        1
        / dist
        * np.where(dist <= cutoff, 1 / cutoff**degree, 1 / dist**degree)
    )
...
def epsilon_compare(point, df):
    ...
    if len(numeric_columns) > 0:
        numeric_diff = (
            np.abs(
                ...
                - point[numeric_columns].to_numpy()
            )
            >= EQ_EPSILON
        )
    ...
    if len(other_columns) > 0:
        other_diff = (
            df[other_columns].to_numpy() != point[other_columns].to_numpy()
        )
...
def random_poi(
    ...
    """
    poi = dataset[dataset[column] == label].sample(1, random_state=seed)
    if drop_label:
