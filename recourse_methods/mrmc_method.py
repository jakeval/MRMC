...
class RecourseRescaler(Protocol):
    ...
    def rescale(mrm: MRM, direction: pd.DataFrame) -> pd.DataFrame:
        pass


def normalize_rescaler(mrm: MRM, direction: pd.DataFrame) -> pd.DataFrame:
...
def get_constant_step_size_rescaler(step_size: float = 1):
    ...
    return lambda _, direction: utils.constant_step_size(direction, step_size=step_size)
...
class MRM:
    ...
    def __init__(self,
        ...
        self.data = MRM.process_data(dataset, adapter, label_column, positive_label)
    ...
    def process_data(dataset: pd.DataFrame,
        ...
        positive_embedded_dataset = adapter.transform(positive_dataset.drop(label_column, axis=1))
...
class MRMC(RecourseMethod):
    ...
    def __init__(self,
        ...
        for cluster_index in range(k_directions):
            ...
            clustered_data = data.loc[indices]
            dataset_cluster = dataset.loc[clustered_data.index]
            mrm = MRM(
    ...
    def cluster_data(self, data: recourse_adapter.EmbeddedDataFrame, k_directions: int) -> Clusters:
        ...
        cluster_assignments = np.vstack([data.index.to_numpy(), cluster_assignments]).T
...
def check_directions(poi: recourse_adapter.EmbeddedSeries, directions: recourse_adapter.EmbeddedDataFrame, cluster_centers: np.ndarray) -> np.ndarray:
    ...
    expected_directions = expected_directions / np.linalg.norm(expected_directions, axis=1)[:,None]
    actual_directions = directions.to_numpy()
    actual_directions = actual_directions / np.linalg.norm(actual_directions, axis=1)[:,None]
